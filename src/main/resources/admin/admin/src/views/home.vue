<template>
	<div class="home-content">
		...
		<div class="statis-box">
			<div id="statis1" class="statis1 animate__animated" v-if="isAuth('mujuanzhe','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-zhangjie8"></span>
				</div>
				<div class="right">
					<div class="num">{{mujuanzheCount}}</div>
					<div class="name">募捐者总数</div>
				</div>
			</div>
			<div id="statis2" class="statis2 animate__animated" v-if="isAuth('juanzengzhe','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-zhangjie8"></span>
				</div>
				<div class="right">
			...
			<div id="statis3" class="statis3 animate__animated" v-if="isAuth('mujuanxiangmu','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-zhangjie8"></span>
				</div>
				<div class="right">
					<div class="num">{{mujuanxiangmuCount}}</div>
					<div class="name">募捐项目总数</div>
				</div>
			</div>
		</div>
		<!-- statis -->
		...
		<!-- 5 -->
		<div class="type5">
			<div id="mujuanzheChart1" class="echarts1 animate__animated" v-if="isAuth('mujuanzhe','首页统计')"></div>
			<div id="juanzengzheChart1" class="echarts2 animate__animated" v-if="isAuth('juanzengzhe','首页统计')"></div>
			<div id="mujuanxiangmuChart1" class="echarts3 animate__animated" v-if="isAuth('mujuanxiangmu','首页统计')"></div>
			<div id="mujuanxiangmuChart2" class="echarts4 animate__animated" v-if="isAuth('mujuanxiangmu','首页统计')"></div>
			<div id="mujuanxiangmuChart3" class="echarts5 animate__animated" v-if="isAuth('mujuanxiangmu','首页统计')"></div>
		</div>
	</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getmujuanzheCount();
		if(this.isAuth('mujuanzhe','首页统计')){
			this.mujuanzheChat1();
		}
		this.getjuanzengzheCount();
		if(this.isAuth('juanzengzhe','首页统计')){
			this.juanzengzheChat1();
		}
		this.getmujuanxiangmuCount();
		if(this.isAuth('mujuanxiangmu','首页统计')){
			this.mujuanxiangmuChat1();
		}
		if(this.isAuth('mujuanxiangmu','首页统计')){
			this.mujuanxiangmuChat2();
		}
		if(this.isAuth('mujuanxiangmu','首页统计')){
			this.mujuanxiangmuChat3();
		}
	...
	methods:{
		handleScroll() {
			let arr = [
				...
				{id:'mujuanzheChart1',css:'animate__'},
				{id:'juanzengzheChart1',css:'animate__'},
				{id:'mujuanxiangmuChart1',css:'animate__'},
				{id:'mujuanxiangmuChart2',css:'animate__'},
				{id:'mujuanxiangmuChart3',css:'animate__'},
			]
...
// 1234 饼2
		mujuanzheChat1() {
			this.$nextTick(()=>{
				...
				var mujuanzheChart1 = echarts.init(document.getElementById("mujuanzheChart1"),'macarons');
				this.$http({
					url: "mujuanzhe/group/xingbie",
					method: "get",
				}).then(({ data }) => {
					if (data && data.code === 0) {
						let res = data.data;
						...
						// 使用刚指定的配置项和数据显示图表。
						mujuanzheChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							mujuanzheChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
...
// 1234 饼3
		juanzengzheChat1() {
			this.$nextTick(()=>{
				...
				var juanzengzheChart1 = echarts.init(document.getElementById("juanzengzheChart1"),'macarons');
				this.$http({
					url: "juanzengzhe/group/xingbie",
					method: "get",
				}).then(({ data }) => {
					if (data && data.code === 0) {
						let res = data.data;
						...
						// 使用刚指定的配置项和数据显示图表。
						juanzengzheChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							juanzengzheChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		getmujuanxiangmuCount() {
			this.$http({
				url: `mujuanxiangmu/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.mujuanxiangmuCount = data.data
				}
			})
		},
// 1234 竖
		mujuanxiangmuChat1() {
			this.$nextTick(()=>{
				...
				var mujuanxiangmuChart1 = echarts.init(document.getElementById("mujuanxiangmuChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						mujuanxiangmuChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							mujuanxiangmuChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		mujuanxiangmuChat2() {
			this.$nextTick(()=>{
				...
				var mujuanxiangmuChart2 = echarts.init(document.getElementById("mujuanxiangmuChart2"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						mujuanxiangmuChart2.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							mujuanxiangmuChart2.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		mujuanxiangmuChat3() {
			this.$nextTick(()=>{
				...
				var mujuanxiangmuChart3 = echarts.init(document.getElementById("mujuanxiangmuChart3"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						mujuanxiangmuChart3.setOption(option);
						//根据窗口的大小变动图表
						window.onresize = function() {
							mujuanxiangmuChart3.resize();
						};
					}else{
						this.$message({
							message: data.msg,
