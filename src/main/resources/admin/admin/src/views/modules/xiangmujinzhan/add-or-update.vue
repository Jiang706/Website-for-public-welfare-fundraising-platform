	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'"  label="项目进展" prop="xiangmujinzhan" >
					<el-select :disabled="ro.xiangmujinzhan" v-model="ruleForm.xiangmujinzhan" placeholder="请选择项目进展" >
						<el-option
							v-for="(item,index) in xiangmujinzhanOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item v-else class="input" label="项目进展" prop="xiangmujinzhan" >
					<el-input v-model="ruleForm.xiangmujinzhan"
						placeholder="项目进展" readonly></el-input>
				</el-form-item>
			...
			<el-form-item v-if="type!='info'"  label="进展详情" prop="jinzhanxiangqing" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.jinzhanxiangqing" 
					class="editor"
					myQuillEditor="jinzhanxiangqing"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.jinzhanxiangqing" label="进展详情" prop="jinzhanxiangqing" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.jinzhanxiangqing"></span>
			</el-form-item>
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='jinzhanxiangqing'){
							this.ruleForm.jinzhanxiangqing = obj[o];
							this.ro.jinzhanxiangqing = true;
							continue;
						}
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.jinzhanxiangqing = this.ruleForm.jinzhanxiangqing.replace(reg,'../../../springboot51ug89lx/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `xiangmujinzhan/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.xiangmujinzhanCrossAddOrUpdateFlag = false;
											this.parent.search();
