		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="目标金额" prop="mubiaojine" >
					<el-input-number v-model="ruleForm.mubiaojine" placeholder="目标金额" :disabled="ro.mubiaojine"></el-input-number>
				</el-form-item>
				<el-form-item v-else class="input" label="目标金额" prop="mubiaojine" >
					<el-input v-model="ruleForm.mubiaojine" placeholder="目标金额" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="已捐金额" prop="yijuanjine" >
					<el-input-number v-model="ruleForm.yijuanjine" placeholder="已捐金额" :disabled="ro.yijuanjine"></el-input-number>
				</el-form-item>
				<el-form-item v-else class="input" label="已捐金额" prop="yijuanjine" >
					<el-input v-model="ruleForm.yijuanjine" placeholder="已捐金额" readonly></el-input>
				</el-form-item>
			...
			<el-form-item class="textarea" v-if="type!='info'" label="项目用途" prop="xiangmuyongtu" >
				<el-input
			...
			<el-form-item v-else-if="ruleForm.xiangmuyongtu" label="项目用途" prop="xiangmuyongtu" >
				<span class="text">{{ruleForm.xiangmuyongtu}}</span>
			</el-form-item>
	...
	export default {
		...
		methods: {
			...
			init(id,type) {
				...
				this.$http({
					...
					if (data && data.code === 0) {
						...
						if(((json.mujuanzhezhanghao!=''&&json.mujuanzhezhanghao) || json.mujuanzhezhanghao==0) && this.$storage.get("role")!="管理员"){
							this.ruleForm.mujuanzhezhanghao = json.mujuanzhezhanghao
							this.ro.mujuanzhezhanghao = true;
						}
						if(((json.mujuanzhexingming!=''&&json.mujuanzhexingming) || json.mujuanzhexingming==0) && this.$storage.get("role")!="管理员"){
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.xiangmuxiangqing = this.ruleForm.xiangmuxiangqing.replace(reg,'../../../springboot51ug89lx/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					if(parseFloat(this.ruleForm.yijuanjine)>parseFloat(this.ruleForm.mubiaojine)){
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `mujuanxiangmu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.mujuanxiangmuCrossAddOrUpdateFlag = false;
											this.parent.search();
