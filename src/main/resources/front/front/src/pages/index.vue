	...
	<div class="main-containers">
		<div class="body-containers">
			<div class="top-container">
				<!-- info -->
				<div class="top_title">
					<span @click="goMenu('/index/home')">基于默克尔树技术的透明化慈善募捐管理信息系统</span>
				</div>
	...
	methods: {
		...
		async getSession() {
			await this.$http.get(`${localStorage.getItem('UserTableName')}/session`, {emulateJSON: true}).then(async res => {
				if (res.data.code == 0) {
					if(localStorage.getItem('UserTableName')== 'mujuanzhe') {
						localStorage.setItem('username', res.data.data.mujuanzhezhanghao);
					}
					if(localStorage.getItem('UserTableName')== 'juanzengzhe') {
						localStorage.setItem('username', res.data.data.juanzengzhanghao);
					}
