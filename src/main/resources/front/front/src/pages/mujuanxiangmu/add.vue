	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="项目类型" prop="xiangmuleixing">
				<el-select v-model="ruleForm.xiangmuleixing" placeholder="请选择项目类型" :disabled=" false  ||ro.xiangmuleixing" >
					<el-option
			...
			<el-form-item class="add-item" label="目标金额" prop="mubiaojine">
				<el-input-number v-model="ruleForm.mubiaojine" placeholder="目标金额" :disabled=" false ||ro.mubiaojine"></el-input-number>
			</el-form-item>
			<el-form-item class="add-item" label="已捐金额" prop="yijuanjine">
				<el-input-number v-model="ruleForm.yijuanjine" placeholder="已捐金额" :disabled=" false ||ro.yijuanjine"></el-input-number>
			</el-form-item>
			...
			<el-form-item class="add-item" label="募捐者账号" prop="mujuanzhezhanghao">
				<el-input v-model="ruleForm.mujuanzhezhanghao" 
					placeholder="募捐者账号" clearable :disabled=" false  ||ro.mujuanzhezhanghao"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="募捐者姓名" prop="mujuanzhexingming">
				<el-input v-model="ruleForm.mujuanzhexingming" 
					placeholder="募捐者姓名" clearable :disabled=" false  ||ro.mujuanzhexingming"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="项目用途" prop="xiangmuyongtu">
				<el-input
					type="textarea"
					:rows="8"
					:disabled=" false  ||ro.xiangmuyongtu"
					placeholder="项目用途"
					v-model="ruleForm.xiangmuyongtu">
					</el-input>
			</el-form-item>
			<el-form-item class="add-item" label="项目详情" prop="xiangmuxiangqing">
				<editor 
...
<script>
	export default {
		data() {
			return {
				...
				rules: {
					...
					yijuanjine: [
						{ required: true, message: '已捐金额不能为空', trigger: 'blur' },
						{ validator: this.$validate.isNumber, trigger: 'blur' },
					],
					zhengmingcailiao: [
					],
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='xiangmuyongtu'){
							this.ruleForm.xiangmuyongtu = obj[o];
							this.ro.xiangmuyongtu = true;
							continue;
						}
				...
				this.$http.get(this.userTableName + '/session', {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						...
						if((json.mujuanzhezhanghao!=''&&json.mujuanzhezhanghao) || json.mujuanzhezhanghao==0){
							this.ruleForm.mujuanzhezhanghao = json.mujuanzhezhanghao;
							this.ro.mujuanzhezhanghao = true;
						}
						if((json.mujuanzhexingming!=''&&json.mujuanzhexingming) || json.mujuanzhexingming==0){
							this.ruleForm.mujuanzhexingming = json.mujuanzhexingming;
							this.ro.mujuanzhexingming = true;
						}
					}
				});
				this.$http.get('option/xiangmuleixing/xiangmuleixing', {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						this.xiangmuleixingOptions = res.data.data;
					}
				});
			...
			// 多级联动参数
			info() {
				this.$http.get(`mujuanxiangmu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			// 提交
			async onSubmit() {
				if(this.ruleForm.yijuanjine>this.ruleForm.mubiaojine){
					this.$message.error("已捐金额不能超过目标金额");
					return
				}
				if(!this.ruleForm.id) {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`mujuanxiangmu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
